# get pendulum balanced at a desired point
...
def main():
    ...
    # --- INITIALIZATION ---
    q_init = np.array([0.0, 1.5708, 0.0, 2 * -1.5708, 1.5708, -1.5708, 2 * 1.5708])
    # q_init = np.pi * 2 * np.random.rand(model.nv) - np.pi
    # Make sure we don't start right on a singularity
    # q_init = q_init + 0.001 * np.random.rand(q_init.shape[0])
    print("q_init: ", q_init)
    ...
    model.dof_damping[:] = 1.0  # disable damping
    ...
    pos_body_ID = ee_body_ID
    rot_body_ID = pend_body_ID
    ...
    # desired point for the end-effector
    y_traj_des = np.array([490e-3, 90e-3, 600e-3])
    y_traj_des = data.xpos[pos_body_ID].copy()
    dy_traj_des = np.zeros_like(y_traj_des)  # zero
    ddy_traj_des = np.zeros_like(y_traj_des)  # zero

    y_size = 3  # size of output, 3 for rotation and 3 for position

    # --- Controller Parameters ---
    # Gains (Tuned for stability)
    lam = -10.0  # eigenvalues for Kp and Kd calculations; no imaginary part
    kp_val = lam * lam
    kd_val = -(lam + lam)
    ...
    jacp_rot = np.zeros((3, model.nv))
    jacr_rot = np.zeros((3, model.nv))

    jacp_pos = np.zeros((3, model.nv))
    jacr_pos = np.zeros((3, model.nv))
    ...
    sig_tol = 1e-6
    warning_flag = False
    lambda_ = 1e-5  # for damping

    # for the circular trajectory
    radius = 0.2  # 0.5m
    constant = 0.3  # planar height
    omega = 2 * np.pi / 100
    ...
    w_reg = 0.1  # Regularize Torques (Minimize Effort)
    ...
    while not glfw.window_should_close(window) and data.time < SIM_DURATION:
        ...
        dy_des = np.concatenate(
            (np.zeros(3), dy_traj_des)
        )  # first 3 elements are desired orientation error (or angular velocity)

        dy_des = np.zeros(3)

        ddy_des = np.concatenate(
            (np.zeros(3), ddy_traj_des)
        )  # first 3 elements are desired orientation error acceration

        ddy_des = np.zeros(3)

        # --- Kinematics ---
        # get current rotation matrix
        R_flat = data.xmat[rot_body_ID].copy()
        ...
        # get current pendulum base position
        pos_curr = data.xpos[pos_body_ID].copy()
        ...
        y = (
            -2 * quat_err[1:]
        )  # this is the error output for orientation; added -sign to change error definition
        # y[3:] = pos_curr - y_traj_des
        # print(y)

        twist_component = np.dot(y, u_global) * u_global

        # y = y - twist_component

        # get site jacobian and calculate J_task
        mujoco.mj_jacBody(model, data, jacp_pos, jacr_pos, pos_body_ID)
        mujoco.mj_jacBody(model, data, jacp_rot, jacr_rot, rot_body_ID)
        # J_task = np.vstack((jacr_rot, jacp_pos))
        J_task = jacr_rot
        ...
        # # RNE trick
        grav_save = model.opt.gravity.copy()
        model.opt.gravity[:] = 0
        data.qacc[:] = 0
        ...
        # # 3. Extract Hybrid Eta (Must match J_task structure!)
        # # Top 3: Pendulum Angular Acceleration Bias
        eta_ang = data.cacc[pend_body_ID][:3].copy()

        # # Bottom 3: Wrist Linear Acceleration Bias
        # # Note: cacc is [rot_acc, lin_acc]. We want indices 3,4,5.
        # eta_lin = data.cacc[ee_body_ID][3:].copy()

        # # Combine
        # eta = np.concatenate((eta_ang, eta_lin))

        eta = eta_ang

        # # 4. Restore Gravity
        model.opt.gravity[:] = grav_save

        # 5. Use 'eta' in your b vector instead of 'dJ_task @ dq'
        # b = nu - eta + (J_task @ M_inv @ h)
        ...
        P[:dim_q, :dim_q] = w_balance * (J_task.T @ J_task)
        q_vec[:dim_q] = -w_balance * J_task.T @ (ddy - eta)

        P[dim_q:, dim_q:] += w_reg * np.eye(dim_tau)  # Damp Torques
        P[:dim_q, :dim_q] += w_posture * np.eye(dim_q)  # Damp Accels
        ...
        lower_bound = np.concatenate(
            [-np.inf * np.ones(dim_q), -tau_lim * np.ones(dim_tau)]
        )
        ...
        if step_count % 1000 == 0:
            ...
            print("y_pos_des:", y_traj_des)
            print("y_pos_curr:", pos_curr)
            # print("quat_err:", quat_err)
            print("y:", y)
        ...
        if time.time() - last_render_time >= 1 / RENDER_FREQ:
            ...
            if scene.ngeom < scene.maxgeom:
                ...
                point_geom = scene.geoms[scene.ngeom]
                mujoco.mjv_initGeom(
                    scene.geoms[scene.ngeom],  # Edit the next available geom directly
