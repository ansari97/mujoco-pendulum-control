...
def main():
    ...
    # --- INITIALIZATION ---
    q_init = np.array([0, -1.5708, 1.5708, -1.5708, 1.5708, -1.5708, 0])
    ...
    model.dof_damping[:] = 2  # disable damping
    ...
    while not glfw.window_should_close(window) and data.time < SIM_DURATION:
        ...
        # b = ddy + J_task @ M_inv @ h_total - eta + ddy_des  # better performance
        A = J_task @ M_inv @ B
        ...
        if cond_A > 10e6:
            print("inside loop rank: ", rank_A)
            print("inside loop cond: ", cond_A)
